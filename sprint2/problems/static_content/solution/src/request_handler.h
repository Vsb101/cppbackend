...
public:
     ...
     * @param static_files_root Путь к корневому каталогу со статическими файлами
     */
    ...
    void operator()(http::request<Body, http::basic_fields<Allocator>>&& req, Send&& send) {
        ...
        if (target.rfind("api/v1/maps", 0) == 0) {
            ...
            HandleStaticFile(req, send, target);
        }
    }
...
private:
    ...
    /**
     * @brief Обрабатывает запрос статического файла
     * @param req Исходный запрос
     * @param send Callback для отправки ответа
     * @param target URI-строка запроса (уже без ведущего /)
     */
    void HandleStaticFile(const http::request<http::string_body>& req, 
                          std::function<void(http::response<http::string_body>&&)>&& send, 
                          const std::string& target);
    
    /**
     * @brief Получает MIME-тип по расширению файла
    ...
    /**
     * @brief Проверяет, что путь находится внутри корневого каталога
     * @param path Путь для проверки
     * @return true если путь валиден, false если вышел за пределы корневого каталога
     */
     ...
     * @param path Путь к файлу
     * @return Содержимое файла или пустую строку если файл не найден
     */
    std::optional<std::string> ReadFileContent(const std::filesystem::path& path) const;

    /**
     * @brief Создаёт HTTP-ответ с содержимым статического файла
     * @param content Содержимое файла
     * @param path Путь к файлу (для определения MIME-типа)
     * @param req Исходный запрос (для keep_alive)
     * @return HTTP-ответ с кодом 200 OK и содержимым файла
     */
    http::response<http::string_body> MakeFileResponse(const std::string& content, 
