...
class CppServer:
    ...
    def __init__(self,
        ...
        try:
            ...
            if docker_network == 'host':
                # При network=host обращаемся по localhost
                ...
                # Иначе - по IP адресу контейнера
                container_info = inspector.inspect_container(self.container.id)
                server_domain = container_info['NetworkSettings']['Networks']['bridge']['IPAddress']
