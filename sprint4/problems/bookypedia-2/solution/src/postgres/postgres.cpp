...
void AuthorRepositoryImpl::Save(const domain::Author& author) {
    ...
    if (current_tx_) { execute(*current_tx_); } 
    else { pqxx::work work{connection_}; execute(work); work.commit(); }
}
...
std::vector<domain::Author> AuthorRepositoryImpl::GetAll() const {
    ...
    if (current_tx_) { execute(*current_tx_); } 
    else { pqxx::read_transaction r{connection_}; execute(r); }
    return result;
}
...
ORDER BY b.title, a.name, b.publication_year, b.id, t.tag;
        ...
        auto push_current_book = [&]() {
            if (current_book_id.has_value()) {
                result.emplace_back(
                    *current_book_id, current_author_id, current_title, current_year, std::move(current_tags)
                );
                current_tags.clear();
            }
        };
        ...
        for (size_t i = 0; i < rows.size(); ++i) {
            auto row_book_id = domain::BookId::FromString(rows[i][0].as<std::string>());
            if (!current_book_id.has_value() || *current_book_id != row_book_id) {
                push_current_book();
                current_book_id = row_book_id;
                current_author_id = domain::AuthorId::FromString(rows[i][1].as<std::string>());
                current_title = rows[i][2].as<std::string>();
                current_year = rows[i][3].as<int>();
            }
            if (!rows[i][4].is_null()) {
                current_tags.push_back(rows[i][4].as<std::string>());
            }
        }
        push_current_book();
    };
