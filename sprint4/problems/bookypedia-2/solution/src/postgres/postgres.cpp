...
namespace postgres {
    ...
    UnitOfWorkImpl::UnitOfWorkImpl(pqxx::connection& connection, app::TypeOfTransaction type_of_tr) 
        ...
        if (type_of_tr == app::TypeOfTransaction::Read) {
            ...
            authors_ = std::make_unique<AuthorRepositoryImpl>(*read_transaction_);
            books_ = std::make_unique<BookRepositoryImpl>(*read_transaction_);
            tags_ = std::make_unique<TagRepositoryImpl>(*read_transaction_);
        } else {
            work_ = std::make_unique<pqxx::work>(connection_);
            authors_ = std::make_unique<AuthorRepositoryImpl>(*work_);
            books_ = std::make_unique<BookRepositoryImpl>(*work_);
            tags_ = std::make_unique<TagRepositoryImpl>(*work_);
        }
    }
