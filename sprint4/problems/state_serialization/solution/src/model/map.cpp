...
void Map::SetLootTypeValue(size_t type, int value) {
    if (loot_type_values_.size() <= type) {
        loot_type_values_.resize(type + 1);
    }
    loot_type_values_[type] = value;
}

int Map::GetLootTypeValue(size_t type) const {
