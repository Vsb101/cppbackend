...
public:
    explicit Application(model::Game game, ExtraData extra_data, bool is_tick_auto, bool randomize_spawn,
                         std::shared_ptr<infra::RetiredPlayerDb> db = nullptr, double dog_retirement_time = 60.0)
        : game_(std::move(game))
        , extra_data_(std::move(extra_data))
        , is_tick_auto_(is_tick_auto)
        , randomize_spawn_(randomize_spawn)
        , retired_player_db_(std::move(db))
    ...
    std::vector<RetiredPlayer> GetRecords(size_t offset, size_t limit);
...
private:
    ...
    std::shared_ptr<infra::RetiredPlayerDb> retired_player_db_;
