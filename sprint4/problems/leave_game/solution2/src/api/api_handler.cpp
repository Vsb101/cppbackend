...
http::response<http::string_body> ApiHandler::HandleRequest(const http::request<http::string_body>& req) {
    ...
    // 8. Таблица рекордов
    {
        auto records_path = api::Endpoints::GetRecords();
        if (target == records_path || (target.starts_with(records_path) && (target.size() == records_path.size() || target[records_path.size()] == '?'))) {
            if (method == http::verb::get || method == http::verb::head) 
                return HandleGetRecords(target);
            return MakeJsonResponseWithAllow(http::status::method_not_allowed, 
                json::value{{"code", "invalidMethod"}, {"message", "Invalid method"}}, "GET, HEAD"sv);
        }
    }
...
http::response<http::string_body> ApiHandler::HandleGetRecords(std::string_view target) {
    constexpr size_t DEFAULT_MAX_ITEMS = 100;
    constexpr size_t MAX_ITEMS_LIMIT = 100;

    size_t start = 0;
    size_t max_items = DEFAULT_MAX_ITEMS;
    ...
    if (qpos != std::string_view::npos) {
        ...
        while (!query.empty()) {
            ...
            std::string_view key = query.substr(0, eq);
            std::string_view val = (amp == std::string_view::npos) ? query.substr(eq + 1) : query.substr(eq + 1, amp - eq - 1);

            try {
                if (key == "start") {
                    start = static_cast<size_t>(std::stoull(std::string(val)));
                } else if (key == "maxItems") {
                    max_items = static_cast<size_t>(std::stoull(std::string(val)));
                }
            } catch (...) {
                return MakeJsonResponse(http::status::bad_request,
                    json::value{{"code", "badRequest"}, {"message", "Invalid query parameters"}});
            }
    ...
    if (max_items > MAX_ITEMS_LIMIT) {
        return MakeJsonResponse(http::status::bad_request,
            json::value{{"code", "badRequest"}, {"message", "maxItems cannot exceed 100"}});
    }
    ...
    for (const auto& rec : records) {
        arr.push_back(json::value_from(rec));
    }
