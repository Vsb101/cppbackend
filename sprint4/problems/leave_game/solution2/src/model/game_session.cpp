...
void GameSession::RemoveDog(Dog::Id id) {
    std::lock_guard lock(mutex_);
    auto it = std::remove_if(dogs_.begin(), dogs_.end(),
...
std::vector<std::shared_ptr<Dog>> GameSession::GetDogs() const noexcept {
    std::lock_guard lock(mutex_);
    return dogs_;
}

std::unordered_map<uint32_t, LostObject> GameSession::GetLostObjects() const noexcept {
    std::lock_guard lock(mutex_);
...
void GameSession::ProcessOfficeReturns() {
    auto loot_types_count = map_->GetLootTypesCount();
    std::vector<int> loot_type_values;
    loot_type_values.reserve(loot_types_count);
    for (size_t i = 0; i < loot_types_count; ++i) {
        loot_type_values.push_back(map_->GetLootTypeValue(i));
    }

    for (auto& dog : dogs_) {
        if (IsDogAtOffice(dog->GetPosition())) {
            dog->ResetIdleTime();  // Собака на базе — не бездействует
            auto returned_items = dog->ClearBag();
            for (const auto& item : returned_items) {
                if (item.type < loot_type_values.size()) {
...
void GameSession::GenerateLoot(std::chrono::milliseconds delta) {
    unsigned new_loot_count = loot_generator_.Generate(delta, static_cast<unsigned>(lost_objects_.size()), static_cast<unsigned>(dogs_.size()));
