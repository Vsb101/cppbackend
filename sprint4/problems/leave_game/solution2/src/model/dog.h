...
public:
    ...
    void UpdateTime(double dt) { play_time_ += dt; }
    void AddIdleTime(double dt) { idle_time_ += dt; }
    void ResetIdleTime() { idle_time_ = 0.0; }
    [[nodiscard]] bool IsRetired(double limit_sec) const { return idle_time_ >= limit_sec; }
    [[nodiscard]] double GetPlayTime() const { return play_time_; }
    [[nodiscard]] double GetIdleTime() const { return idle_time_; }
