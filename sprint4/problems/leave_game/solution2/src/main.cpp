...
#include "infra/db_connection_pool.h"
#include "infra/retired_player_db.h"
    ...
    timer->async_wait(net::bind_executor(*strand, [timer, period, app, strand](sys::error_code ec) {
        if (!ec) {
            try {
                ...
                logger::LogError("tick_failed"sv, e.what());
            }
...
int main(int argc, const char* argv[]) {
    ...
    try {
        ...
        std::shared_ptr<infra::RetiredPlayerDb> retired_db;
        const char* db_url = std::getenv("GAME_DB_URL");
        if (db_url) {
            try {
                auto pool = std::make_shared<infra::ConnectionPool>(
                    num_threads,
                    [db_url] { return std::make_shared<pqxx::connection>(db_url); }
                );
                retired_db = std::make_shared<infra::RetiredPlayerDb>(pool);
                logger::LogInfo("database connected"sv);
            } catch (const std::exception& e) {
                logger::LogError("db_connection_failed"sv, e.what());
                // Продолжаем без БД
            }
        }
            ...
            workers.emplace_back([&ioc] {
                while (true) {
                    try {
                        ...
                        logger::LogError("worker_exception"sv, e.what());
                    }
                }
            });
        }
        while (true) {
            try {
                ...
                logger::LogError("main_loop_exception"sv, e.what());
            }
        }
