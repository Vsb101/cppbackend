...
// Анонимный namespace для констант и настроек по умолчанию
namespace {

constexpr double DEFAULT_DOG_SPEED = 1.0;
constexpr double DEFAULT_DOG_RETIREMENT_TIME = 60.0; // 1 минута в секундах согласно ТЗ
constexpr size_t DEFAULT_BAG_CAPACITY = 3;

constexpr double DEFAULT_LOOT_PERIOD = 1.0;
constexpr double DEFAULT_LOOT_PROBABILITY = 0.1;
...
ProjectConfig LoadGame(const std::filesystem::path& json_path) {
    ...
    if (auto it = root_obj.find("lootGeneratorConfig"); it != root_obj.end()) {
        ...
        config.game.SetLootGeneratorConfig({DEFAULT_LOOT_PERIOD, DEFAULT_LOOT_PROBABILITY});
    }
    ...
    // Время бездействия собак до удаления (в секундах)
    double dog_retirement_time = DEFAULT_DOG_RETIREMENT_TIME;
    if (auto it = root_obj.find("dogRetirementTime"); it != root_obj.end()) {
        dog_retirement_time = it->value().as_double();
    }
    config.game.SetDogRetirementTime(dog_retirement_time);

    // Вместимость рюкзака по умолчанию
    size_t default_bag_capacity = DEFAULT_BAG_CAPACITY;
