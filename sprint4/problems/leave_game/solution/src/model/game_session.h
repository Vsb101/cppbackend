...
class GameSession {
 public:
    ...
    // Сигнатура функции обратного вызова при выходе собаки на заслуженный отдых
    using DogRetirementHandler = std::function<void(const std::shared_ptr<Dog>&)>;
    ...
    // Обновленный метод Update, принимающий лимит бездействия и callback на удаление
    void Update(double dt_seconds, double retirement_time_limit, const DogRetirementHandler& on_dog_retired);
 ...
 private:
    ...
    // --- Проверка ухода на пенсию ---
    void RetireIdleDogs(double retirement_time_limit, const DogRetirementHandler& on_dog_retired);
