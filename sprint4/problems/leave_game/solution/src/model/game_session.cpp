...
void GameSession::Update(double dt_seconds, double retirement_time_limit, const DogRetirementHandler& on_dog_retired) {
    std::lock_guard lock(mutex_);

    // Обновляем игровое время и время бездействия для всех собак
    for (auto& dog : dogs_) {
    ...
    //Расчет движения и коллизий
    auto movements = PrepareDogMovements();
    MoveDogs(dt_seconds, movements);
    ProcessGatherEvents(movements);

    // Проверяем и удаляем собак, ушедших на покой
    RetireIdleDogs(retirement_time_limit, on_dog_retired);
}
...
void GameSession::RetireIdleDogs(double retirement_time_limit, const DogRetirementHandler& on_dog_retired) {
    auto it = dogs_.begin();
    while (it != dogs_.end()) {
        if ((*it)->GetIdleTime() >= retirement_time_limit) {
            // Если задан callback — вызываем его перед удалением объекта
            if (on_dog_retired) {
                on_dog_retired(*it);
            }
            it = dogs_.erase(it);
