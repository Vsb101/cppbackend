...
SCENARIO("Application state serialization - basic") {
    GIVEN("an application with game state") {
        ...
        // Явно передаем пустую строку в качестве db_url, чтобы точно сопоставить конструктор
        ...
        // Двигаем собаку и устанавливаем начальные таймеры времени
        dog->SetPosition({10.0, 10.0});
        dog->SetDirection(model::Direction::EAST);
        (void)dog->AddToBag(1, 0); // Избавляемся от предупреждений nodiscard
        dog->AddScore(100);
        
        // Симулируем, что собака бегала и стояла, чтобы проверить новые поля времени
        dog->UpdateTime(15.5); 
        ...
        WHEN("serializing state") {
            ...
            WHEN("deserializing state to new application") {
                ...
                THEN("state is restored") {
                    ...
                    // ПРОВЕРКА: Новые счетчики времени успешно восстановились из сериализованного состояния
                    REQUIRE(restored_dog->GetPlayTime() == 15.5);
                    REQUIRE(restored_dog->GetIdleTime() == 15.5); // Скорость была 0, так что idle_time == play_time
                }
...
SCENARIO("Atomic save with temp file") {
    GIVEN("a state that will be saved") {
        ...
        auto& game = const_cast<model::Game&>(app->GetGame());
        auto session = game.FindOrCreateSession(model::Map::Id{"test_map"s});
        (void)session->CreateDog("dog1"s); // Избавляемся от предупреждений nodiscard
