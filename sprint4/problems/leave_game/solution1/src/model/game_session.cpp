...
void GameSession::Update(double dt_seconds, double current_time) {
    std::lock_guard lock(mutex_);
    ...
    MoveDogs(dt_seconds, current_time, movements);
...
void GameSession::MoveDogs(double dt_seconds, double current_time, std::vector<DogMovement>& movements) {
    for (size_t idx = 0; idx < dogs_.size(); ++idx) {
        ...
        if (speed.vx == 0 && speed.vy == 0) {
            if (!dog->IsInactive()) {
                dog->StartInactivity(current_time - dt_seconds);
            }
            continue;
        }
        ...
        while (remaining_dt > EPS) {
            ...
            // Если собака начала двигаться, сбрасываем бездействие
            if (dog->IsInactive()) {
                dog->StopInactivity();
            }
            ...
            if (std::abs(clamped_pos.x - potential_next_pos.x) > EPS ||
                ...
                if (!found_new_road) {
                    dog->SetSpeed({0, 0});
                    if (!dog->IsInactive()) {
                        dog->StartInactivity(current_time);
                    }
                    break;
                }
            } else {
...
void GameSession::RestoreDogs(const std::vector<Dog>& dogs, uint32_t next_dog_id) {
    ...
    for (const auto& dog : dogs) {
        ...
        new_dog->SetJoinTime(dog.GetJoinTime());
        if (dog.IsInactive()) {
            new_dog->StartInactivity(dog.GetInactivityStartTime());
        }
...
void GameSession::RemoveDog(const Dog::Id& id) {
    ...
    dogs_.erase(
        std::remove_if(dogs_.begin(), dogs_.end(),
            [&id](const std::shared_ptr<Dog>& dog) { return dog->GetId() == id; }),
        dogs_.end()
    );
}
